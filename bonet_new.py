...
class BoNet:
	...
	def backbone_pointnet2_first(self, X_pc):
		self.l0_xyz = X_pc[:,:,0:3]
		self.l0_points = X_pc[:,:,3:9]
		self.l0_xyz_expand=tf.expand_dims(self.l0_xyz,2)
		...
		#print(R)
			self.l0_xyz=tf.squeeze(tf.matmul(self.l0_xyz_expand,r))

		return 0
	
	def backbone_pointnet2(self,X_pc,is_train=None):
		points_num = tf.shape(input=X_pc)[1]
		l3_points = pnet2.pointnet_fp_module(self.l3_xyz, self.l4_xyz, self.l3_points, self.l4_points, [256, 256], is_training=None, bn_decay=None, scope='fa_layer1')
		l2_points = pnet2.pointnet_fp_module(self.l2_xyz, self.l3_xyz, self.l2_points, l3_points, [256, 256], is_training=None, bn_decay=None,scope='fa_layer2')
		l1_points = pnet2.pointnet_fp_module(self.l1_xyz, self.l2_xyz, self.l1_points, l2_points, [256, 128], is_training=None, bn_decay=None,scope='fa_layer3')
		l0_points = pnet2.pointnet_fp_module(self.l0_xyz, self.l1_xyz, tf.concat([self.l0_xyz, self.l0_points], axis=-1),
                l1_points,[128, 128, 128, 128], is_training=None, bn_decay=None, scope='fa_layer4')
		global_features = tf.reshape(self.l4_points, [-1, 512])
	...
	def build_graph(self, GPU='0'):
		...
		self.R0 = tf.compat.v1.placeholder(shape=[None,None,3,3],dtype=tf.float32, name='R0')
		self.grouped_xyz0 = tf.compat.v1.placeholder(shape=[None,None,None,3],dtype=tf.float32, name='grouped_xyz0')
		self.new_xyz0 = tf.compat.v1.placeholder(shape=[None,None,3],dtype=tf.float32, name='new_xyz0')
		self.R1 = tf.compat.v1.placeholder(shape=[None,None,3,3],dtype=tf.float32, name='R1')
		self.grouped_xyz1 = tf.compat.v1.placeholder(shape=[None,None,None,3],dtype=tf.float32, name='grouped_xyz1')
		self.new_xyz1 = tf.compat.v1.placeholder(shape=[None,None,3],dtype=tf.float32, name='new_xyz1')
		self.R2 = tf.compat.v1.placeholder(shape=[None,None,3,3],dtype=tf.float32, name='R2')
		self.grouped_xyz2 = tf.compat.v1.placeholder(shape=[None,None,None,3],dtype=tf.float32, name='grouped_xyz2')
		self.new_xyz2 = tf.compat.v1.placeholder(shape=[None,None,3],dtype=tf.float32, name='new_xyz2')
		...
		with tf.compat.v1.variable_scope('backbone'):
			...
			self.backbone_pointnet2_first(self.X_pc)
			self.R0,self.new_xyz0,self.grouped_xyz0,idx0=pnet2.sample_first(1024,0.1,32,self.l0_xyz,False)
			self.l1_xyz, self.l1_points, _ = pnet2.pointnet_sa_module(self.l0_xyz, self.l0_points,self.R0,self.new_xyz0,self.grouped_xyz0,idx0, npoint=1024, radius=0.1, nsample=32,
				mlp=[32, 32, 64], mlp2=None, group_all=False, is_training=None, bn_decay=None, scope='layer1')
			
			self.R1,self.new_xyz1,self.grouped_xyz1,idx1=pnet2.sample_first(256,0.2,64,self.l1_xyz,False)
			self.l2_xyz, self.l2_points, _ = pnet2.pointnet_sa_module(self.l1_xyz, self.l1_points,self.R1,self.new_xyz1,self.grouped_xyz1,idx1, npoint=256, radius=0.2, nsample=64,
				mlp=[64, 64, 128], mlp2=None, group_all=False, is_training=None, bn_decay=None, scope='layer2')
			
			self.R2,self.new_xyz2,self.grouped_xyz2,idx2=pnet2.sample_first(64,0.4,128,self.l2_xyz,False)
			self.l3_xyz, self.l3_points, _ = pnet2.pointnet_sa_module(self.l2_xyz, self.l2_points,self.R2,self.new_xyz2,self.grouped_xyz2,idx2, npoint=64, radius=0.3, nsample=128,
				mlp=[128, 128, 256], mlp2=None, group_all=False, is_training=None, bn_decay=None, scope='layer3')
			
			self.l4_xyz, self.l4_points, _ = pnet2.pointnet_sa_module_all(self.l3_xyz, self.l3_points, npoint=None, radius=None, nsample=None,
