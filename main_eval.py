...
class Evaluation:
	...
	def evaluation(dataset_path, train_areas, result_path):
		...
		for scene_name in res_scenes:
			...
			np.savetxt(result_path +'pc/' +'_'+scene_name + '.txt', pc_all, fmt = '%f', delimiter = ',')
			np.savetxt(result_path +'ins_gt_all/' +'_'+scene_name + '.txt', ins_gt_all, fmt = '%f', delimiter = ',')
			np.savetxt(result_path +'sem_pred_all/' +'_'+scene_name + '.txt', sem_pred_all, fmt = '%f', delimiter = ',')
			np.savetxt(result_path +'sem_gt_all/' +'_'+scene_name + '.txt', sem_gt_all, fmt = '%f', delimiter = ',')
			pc_xyz_int = (pc_all[:, 6:9] / gap).astype(np.int32)
			ins_pred_all = volume[tuple(pc_xyz_int.T)]
			np.savetxt(result_path +'ins_pred_all/' +'_'+scene_name + '.txt', ins_pred_all, fmt = '%f', delimiter = ',')
...
if __name__=='__main__':
	...
	Evaluation.ttest(net, data, result_path, test_batch_size=8)
