...
def sample_first(npoint,radius,nsample,xyz,knn=False):
    ...
    with tf.device('/gpu:0'):
        ...
        _, _, vector = tf.linalg.svd(grouped_xyz_cov)  # SVD分解
        R = convert_p(vector, 4, npoint)
    return R,new_xyz,grouped_xyz,idx

def sample_and_group(npoint, radius, nsample, xyz, points, R,new_xyz,grouped_xyz,idx,knn=False, use_xyz=True,):
...
def pointnet_sa_module(xyz, points,R,new_xyz,grouped_xyz,idx,  npoint, radius, nsample,mlp, mlp2, group_all, is_training, bn_decay, scope,
    ...
    with tf.compat.v1.variable_scope(scope) as sc:
        ...
        if group_all:
            ...
            new_xyz, new_points, idx, grouped_xyz = sample_and_group(npoint, radius, nsample, xyz, points,R,new_xyz,grouped_xyz,idx, knn, use_xyz)
...
def pointnet_sa_module_all(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope,
                       bn=True, pooling='max', knn=False, use_xyz=True, use_nchw=False):
